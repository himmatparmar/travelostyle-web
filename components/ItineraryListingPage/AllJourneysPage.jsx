...
export default function AllJourneysPage() {
      ...
      try {
        ...
        const drupalJourneys = json.data.map((item, index) => {
          ...
          const imageUrl = rawUrl
            ? `http://travelostyle-drupal-backend.ddev.site${rawUrl}`
            : "/GoldenTriange.svg";
          ...
          let viewTripUrl = `/journeys/${item.id}`;

          if (cta?.uri?.startsWith("entity:node/")) {
            const nodeId = cta.uri.replace("entity:node/", "");
            viewTripUrl = `/journeys/${nodeId}`;
          } else if (cta?.uri) {
            viewTripUrl = cta.uri;
          }
  ...
  const popularJourneys = journeys.slice(0, 3);
